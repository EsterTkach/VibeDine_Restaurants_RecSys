...
def get_filtered_restaurants_repo(
    ...
    if accessibility: query["accessibility"] = {"$in": accessibility}
    if service_options: query["service_options"] = {"$in": service_options}
    if atmosphere: query["atmosphere"] = {"$in": atmosphere}
    if dining_options: query["dining_options"] = {"$in": dining_options}
    if crowd: query["crowd"] = {"$in": crowd}
    if offerings: query["offerings"] = {"$in": offerings}
    ...
    for field in query.keys():
        if field not in ["avg_rating", "num_of_reviews"]:  # Prevent projection duplication errors
            projection[field] = 1
    ...
    def add_intersection(field_name, user_list):
        return {
            "$size": {
                "$setIntersection": [
                    {"$ifNull": [f"${field_name}", []]},  # Fallback to empty array if DB field is missing
                    user_list
                ]
            }
        }

    if categories: score_components.append(add_intersection("category", categories))
    if accessibility: score_components.append(add_intersection("accessibility", accessibility))
    if service_options: score_components.append(add_intersection("service_options", service_options))
    if atmosphere: score_components.append(add_intersection("atmosphere", atmosphere))
    if dining_options: score_components.append(add_intersection("dining_options", dining_options))
    if crowd: score_components.append(add_intersection("crowd", crowd))
    if offerings: score_components.append(add_intersection("offerings", offerings))
    ...
    if score_components:
        pipeline.append({
            "$addFields": {
                "match_score": {"$add": score_components}
            }
        })
        pipeline.extend([
